...
class TrajectoryActionController{
    ...
    public:
        TrajectoryActionController(std::string ns, std::string name) : action_name(ns + name), 
            server_(nh_,ns + name,boost::bind(&TrajectoryActionController::executeCB,this,_1),false),
            orientation_client_(ns + "/action/pose",true){
                ...
                vel_pub = nh_.advertise<geometry_msgs::Twist>(ns + "/cmd_vel",10);
                pose_sub = nh_.subscribe<nav_msgs::Odometry>(ns + "/ground_truth/state",10,&TrajectoryActionController::poseCallback,this);
                ros::ServiceClient enable_motors = nh_.serviceClient<hector_uav_msgs::EnableMotors>(ns + "/enable_motors");
...
//   ROS_WARN("Trajectory server");
//   ROS_INFO_STREAM(ns);
  TrajectoryActionController controller(ns.c_str(), "/action/trajectory");
  controller.idle();
  //ros::spin();
  return 0;
}